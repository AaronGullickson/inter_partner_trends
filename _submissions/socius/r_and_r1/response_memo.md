...
Thank you for pushing on this point. There are indeed several limitations in dealing with the same-sex couples that initially led me to restrict the analysis to opposite-sex couples. However, since 2013, it is possible to identify same-sex marriages in the ACS data. Therefore, I have decided to include in the revision an analysis of trends in interracial marriage for same-sex couples over this time period. To do this I have altered the discussion of cohabitation at the end of the results section to broaden the scope to "other union types." For the main paper, I focus on the results for marriages involving one White partner, because of serious sample size limitations, but I do show the full results for both cohabitation and marriage in the supplementary appendix. (TODO: add these figures and discussion to the actual paper). As expected from prior work, I generally find that interracial pairing is more common in same-sex marriages than opposite-sex marriages.

I also provide more detail in the data and methods section explaining the more complicated issues with the same-sex data (TODO: give page number) and why I model this group separately from the heterosexual group. As the reviewer suggests one of the issue with same-sex marriages is that they don't have the same logic of men's characteristics in one dimension and women's characteristics in another dimension that applies to log-linear models. I have substituted this with the household head and non-household head because these cases or only identified when one members is the household head. Since all of my models are symmetric with regard to "gender", this distinction is merely a convenience.

As the reviewer also suggests, it is possible to identify same-sex unmarried partners back to 1990. However, the data quality issues here get quite a bit more complicated and murky. Handling those appropriately would add a great deal of technical detail to an already full analysis, and so I have chosen to restrict the focus only on comparing same-sex to opposite-sex marriages in periods where they are both identifiable.
